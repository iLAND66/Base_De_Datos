...
#directory
#pers sin fam, pers que sob y sobn't
...
sum(is.na(data.combined$Age))               # Contar valores NA
sum(is.infinite(data.combined$Age))        # Contar valores infinitos

data.combined <- data.combined[is.finite(data.combined$Age), ]

table(data.combined$Sex)

mean_age <- mean(data.combined$Age, na.rm = TRUE)  # Calcular la media omitiendo NA
data.combined$Age[is.na(data.combined$Age)] <- mean_age  # Imputar la media
...
nrow(data.combined)  # Debería ser 1046
length(tam.fam) 
ggplot(data.combined[1:891, ], aes(x = Age, fill = Survived)) +
  geom_histogram(binwidth = 5) +  # Ajusta el binwidth según lo necesites
  facet_wrap(~Sex + Pclass) +
